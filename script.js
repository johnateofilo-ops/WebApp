...
function normalizeIngredient(ingredient) {
  return {
    ...
    createdAt: ingredient.createdAt || new Date().toISOString().slice(0, 10)
  };
}
...
function getMonthOptions() {
  ...
  ingredients.forEach(item => {
    if (item.createdAt) {
      months.add(item.createdAt.slice(0, 7));
    }
  });
...
function getWeeklyBreakdown(monthKey, type) {
  const records = type === 'ingredient'
    ? ingredients.filter(item => item.createdAt && item.createdAt.slice(0, 7) === monthKey)
  ...
  records.forEach(record => {
    const date = type === 'ingredient' ? record.createdAt : record.date;
    const weekIndex = Math.ceil(new Date(date).getDate() / 7);
    const label = `Week ${weekIndex}`;
    const current = weekMap.get(label) || 0;
    weekMap.set(label, current + (type === 'ingredient' ? Number(record.baseCost || 0) : Number(record.totalAmount || 0)));
  });
...
ingredientForm.addEventListener('submit', event => {
  ...
  const ingredient = {
    ...
    currentStock: Number(ingredientStockInput.value || 0),
