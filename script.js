...
const adminIngredientsTableBody = document.getElementById('admin-ingredients-table-body');
...
async function applyFirebaseConnection() {
  try {
    ...
    await Promise.all([loadProducts(), loadOrders(), loadOrderItems(), loadIngredients(), loadRecipes()]);
    renderReports();
...
function openIngredientModal(ingredient = null) {
  editingIngredientId = ingredient ? ingredient.ingredientId : null;
  ingredientModalTitle.textContent = ingredient ? 'Edit Ingredient' : 'Add Ingredient';
  ingredientIdInput.value = ingredient ? ingredient.ingredientId : generateNextIngredientId();
  ingredientNameInput.value = ingredient ? ingredient.name : '';
  ingredientCategoryInput.value = ingredient ? (ingredient.category || '') : '';
  ingredientStockInput.value = ingredient ? (ingredient.currentStock || 0) : '';
  ingredientUnitInput.value = ingredient ? (ingredient.unit || '') : '';
  ingredientBaseQtyInput.value = ingredient ? (ingredient.baseQuantity || 0) : '';
  ingredientBaseCostInput.value = ingredient ? (ingredient.baseCost || 0) : '';
  ingredientActiveInput.value = ingredient && ingredient.isActive === false ? 'false' : 'true';
...
function getIngredientCostEvents(monthKey = '') {
  ...
  ingredients.forEach(item => {
    if (Array.isArray(item.restockHistory) && item.restockHistory.length) {
      item.restockHistory.forEach(entry => {
        const timestamp = String(entry.timestamp || '').trim();
        if (!timestamp) return;
        if (monthKey && timestamp.slice(0, 7) !== monthKey) return;

        const quantityAdded = Number(entry.quantityAdded || 0);
        const unitCostSnapshot = Number(entry.unitCostSnapshot || item.unitCost || 0);
        const computedCost = Number(entry.costAdded || quantityAdded * unitCostSnapshot);
        if (quantityAdded <= 0 || computedCost <= 0) return;

        events.push({
          timestamp,
          costAdded: computedCost
        });
      });
      return;
    }

    // Legacy fallback for records without explicit restock history.
    if (item.createdAt && (!monthKey || item.createdAt.slice(0, 7) === monthKey)) {
...
function renderAdminIngredients() {
  adminIngredientsTableBody.innerHTML = ingredients.map(item => `
    <tr>
      <td>${item.ingredientId}</td>
      <td>${item.name}</td>
      <td>${item.category || 'N/A'}</td>
      <td>${Number(item.currentStock || 0).toLocaleString('en-PH', { maximumFractionDigits: 1 })} ${item.unit || ''}</td>
      <td>${item.unit || 'N/A'}</td>
      <td>
        <span class="badge ${item.isActive === false ? 'bg-secondary-subtle text-secondary' : 'bg-success-subtle text-success'} me-2">${item.isActive === false ? 'Inactive' : 'Active'}</span>
        <button class="btn btn-sm btn-outline-secondary edit-ingredient-btn" data-id="${item.ingredientId}" type="button">Edit</button>
      </td>
    </tr>
  `).join('');

  adminIngredientsTableBody.querySelectorAll('.edit-ingredient-btn').forEach(button => {
    button.addEventListener('click', () => {
      const ingredient = ingredients.find(item => item.ingredientId === button.dataset.id);
...
function getWeeklyBreakdown(monthKey, type) {
  ...
  records.forEach(record => {
    const date = type === 'ingredient' ? record.timestamp : record.date;
    const dayOfMonth = Number(String(date).slice(8, 10));
    const weekIndex = dayOfMonth > 0 ? Math.ceil(dayOfMonth / 7) : 1;
    const label = `Week ${weekIndex}`;
    const current = weekMap.get(label) || 0;
    weekMap.set(label, current + (type === 'ingredient' ? Number(record.costAdded || 0) : Number(record.totalAmount || 0)));
  });

  const sortedWeeks = [...weekMap.entries()].sort((a, b) => Number(a[0].split(' ')[1]) - Number(b[0].split(' ')[1]));
  return sortedWeeks.map(([label, value]) => ({ label, value }));
}
...
ingredientForm.addEventListener('submit', event => {
  event.preventDefault();
  const ingredientId = ingredientIdInput.value.trim() || editingIngredientId || generateNextIngredientId();
  const existingIngredient = ingredients.find(item => item.ingredientId === ingredientId) || null;
  ...
  const nextStock = Number(ingredientStockInput.value || 0);
  const previousStock = Number(existingIngredient?.currentStock || 0);
  const quantityAdded = existingIngredient ? Math.max(0, nextStock - previousStock) : Math.max(0, nextStock);
  const nowIso = new Date().toISOString();
  const restockHistory = Array.isArray(existingIngredient?.restockHistory)
    ? [...existingIngredient.restockHistory]
    : [];
  ...
  if (quantityAdded > 0 && unitCost > 0) {
    restockHistory.push({
      ...
      costAdded: quantityAdded * unitCost,
      source: existingIngredient ? 'stock_adjustment' : 'ingredient_create'
    });
  }
  ...
  const ingredient = {
    ...
    baseQuantity,
    baseCost,
    createdAt: existingIngredient?.createdAt || nowIso.slice(0, 10),
    lastRestockedAt: quantityAdded > 0 && unitCost > 0 ? nowIso : (existingIngredient?.lastRestockedAt || ''),
    restockHistory,
    isActive: ingredientActiveInput.value === 'true'
  };

  const existingIndex = ingredients.findIndex(item => item.ingredientId === ingredient.ingredientId);
  if (existingIndex >= 0) {
