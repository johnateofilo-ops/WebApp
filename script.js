...
async function unlockApp() {
  ...
  // so an admin can sign in locally and then supply Firebase config.
  if (!firebaseAuth) {
    const localPin = localStorage.getItem(LOCAL_UNLOCK_PIN_KEY) || '';
    const isLocalFallback = email.toLowerCase() === 'local';
    if (isLocalFallback && (localPin ? password === localPin : password === 'admin')) {
      isLocked = false;
      closePinScreen();
      renderView(pendingViewName || 'pos');
      lockButton.textContent = 'Sign Out (local)';
      setAuthStatus('Locally unlocked (no Firebase).', 'success');
      // Reveal Firebase config controls so admin can apply runtime config
      enableFirebaseConfigUi(true);
      return;
    }

    setAuthStatus('Firebase Authentication is unavailable. Provide config first or use local unlock.', 'warning');
    return;
  }
