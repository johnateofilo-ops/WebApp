...
const dashboardWeekSelect = document.getElementById('dashboard-week-select');
...
const ordersMonthSelect = document.getElementById('orders-month-select');
const ordersWeekSelect = document.getElementById('orders-week-select');
...
const LOCAL_UNLOCK_PIN_KEY = 'coffee-pos-local-unlock-pin';

function enableFirebaseConfigUi(enabled) {
  if (firebaseConfigJsonInput) {
    firebaseConfigJsonInput.style.display = enabled ? '' : 'none';
    firebaseConfigJsonInput.disabled = !enabled;
  }
  if (firebaseDatabaseUrlInput) {
    firebaseDatabaseUrlInput.style.display = enabled ? '' : 'none';
    firebaseDatabaseUrlInput.disabled = !enabled;
  }
  if (applyFirebaseConnectionButton) {
    applyFirebaseConnectionButton.style.display = enabled ? '' : 'none';
    applyFirebaseConnectionButton.disabled = !enabled;
  }
  if (firebaseConnectionStatus) {
    firebaseConnectionStatus.style.display = enabled ? '' : 'none';
  }
}

// Initialize Firebase for Auth only (no database) using a minimal bootstrap config.
function initFirebaseAuthOnly(bootstrapConfig = {}) {
  try {
    const app = initializeApp(bootstrapConfig, `coffee-pos-auth-only-${Date.now()}`);
    firebaseAuth = getAuth(app);
    // do not initialize database here; wait for full runtime config
    attachAuthObserver();
    return firebaseAuth;
  } catch (error) {
    console.error('Auth-only Firebase initialization failed.', error);
    return null;
  }
}

// Try to fetch full runtime config from a server endpoint after sign-in.
async function fetchRuntimeConfigFromServer(idToken) {
  const endpoints = ['/get-firebase-config', '/.netlify/functions/getFirebaseConfig'];
  for (const url of endpoints) {
    try {
      ...
      if (payload && payload.apiKey && payload.projectId) {
        return payload;
      }
    } catch (e) {
      // ignore and try next
...
// Try to fetch a minimal bootstrap config (auth-only) from a public server endpoint
async function fetchBootstrapConfigFromServer() {
  const endpoints = ['/get-bootstrap-config', '/.netlify/functions/getBootstrapConfig'];
  for (const url of endpoints) {
    try {
      const resp = await fetch(url, { method: 'GET', headers: { Accept: 'application/json' } });
      if (!resp.ok) continue;
      const payload = await resp.json();
      if (payload && payload.apiKey && payload.authDomain) {
        return payload;
      }
    } catch (e) {
      // ignore and try next
...
function attachAuthObserver() {
  ...
  authObserverUnsubscribe = onAuthStateChanged(firebaseAuth, user => {
    if (user) {
      (async () => {
        setAuthStatus('Signed in. Fetching runtime config...', 'muted');
        try {
          const idToken = await user.getIdToken(/* forceRefresh */ false);
          // attempt to get runtime config from server endpoint
          const serverConfig = await fetchRuntimeConfigFromServer(idToken);
          if (serverConfig) {
            // store and initialize full firebase
            localStorage.setItem(firebaseRuntimeConfigKey, JSON.stringify(serverConfig));
            activeFirebaseConfig = serverConfig;
            firebaseDb = initFirebase(serverConfig);
            setFirebaseConnectionStatus(`Connected to Firebase project: ${getFirebaseProjectLabel(serverConfig)}`, 'success');
            enableFirebaseConfigUi(false);
          } else {
            // no server config available; reveal config UI so admin can paste it
            enableFirebaseConfigUi(true);
            setFirebaseConnectionStatus('Authenticated but runtime config not found on server. Paste config to continue.', 'warning');
          }
          ...
          console.error('Post-sign-in runtime config fetch failed.', error);
          setAuthStatus('Signed in but unable to obtain runtime config.', 'warning');
          enableFirebaseConfigUi(true);
          isLocked = false;
          closePinScreen();
          renderView(pendingViewName || 'pos');
...
// Do NOT auto-initialize Firebase on script load. Admins must supply
// runtime configuration via the UI (hidden until unlock) or the app
// can be initialized from saved runtime config after an admin unlocks.
...
function readStoredArray(key, fallback = []) {
  try {
    ...
    // Filter out null/undefined entries that may have been introduced
    // by JSON.stringify of arrays containing `undefined` or by
    // older code paths that left gaps in the array.
...
function syncFirebaseCollection(key, value) {
  ...
  try {
    // Firebase Realtime Database rejects values that contain `undefined`.
    // Serialize via JSON to strip/convert undefined into nulls and remove
    // any unserializable values before sending to the database.
    const safeValue = JSON.parse(JSON.stringify(value));
    set(ref(firebaseDb, collection), safeValue).catch(error => {
      console.error(`Failed to sync ${collection} to Firebase.`, error);
    });
  } catch (error) {
    console.error(`Failed to prepare ${collection} for Firebase sync.`, error);
  }
}
...
async function loadDataCollection(storageKey, seedPath) {
  ...
  if (firebaseDb) {
    try {
      ...
      if (!snapshot.exists()) {
        ...
        const normalizedSeed = normalizeCollectionRecords(seedData);
        cacheStoredArray(storageKey, normalizedSeed);
        return normalizedSeed;
      }

      const value = snapshot.val();
      const rawRecords = Array.isArray(value) ? value : Object.values(value || {});
      const records = normalizeCollectionRecords(rawRecords);
  ...
  const dataSource = storedData.length ? storedData : seedData;
  const data = normalizeCollectionRecords(dataSource);
...
function populateIngredientOptionLists() {
  ...
  try { enableTouchComboboxFallback(); } catch (e) { /* ignore */ }
}
...
function fillRecipeRowsForSelection(productName = recipeModalProductSelect.value) {
  ...
  // update per-row costs for any prefilled rows, then update total cost preview
  updateAllRecipeRowCosts();
  refreshRecipeCostPreview();
  // log any issues (missing ingredient matches or missing unitCost)
  logRecipeRowIssues();
}

function logRecipeRowIssues() {
  const rows = Array.from(recipeRowsContainer.querySelectorAll('.recipe-row'));
  if (!rows.length) {
    console.info('checkRecipeUnitCosts: no recipe rows to inspect');
    return;
  }

  rows.forEach((row, index) => {
    const ingredientName = String(row.querySelector('.recipe-ingredient-input')?.value || '').trim();
    const qty = Number(row.querySelector('.recipe-qty-input')?.value || 0) || 0;
    if (!ingredientName) {
      console.warn(`checkRecipeUnitCosts: Row ${index + 1} has empty ingredient name (qty ${qty})`);
      return;
    }

    const ingredient = ingredients.find(it => String(it.name || '').toLowerCase() === ingredientName.toLowerCase());
    if (!ingredient) {
      console.warn(`checkRecipeUnitCosts: Row ${index + 1} — ingredient '${ingredientName}' not found in ingredients[]`);
      return;
    }

    const unitCost = Number(ingredient.unitCost || 0);
    if (!Number.isFinite(unitCost) || unitCost <= 0) {
      console.warn(`checkRecipeUnitCosts: Row ${index + 1} — ingredient '${ingredientName}' has missing/zero unitCost: ${ingredient.unitCost}`);
    } else {
      console.info(`checkRecipeUnitCosts: Row ${index + 1} — '${ingredientName}' unitCost ${unitCost}, qty ${qty}, line ${formatCurrency(unitCost * qty)}`);
    }
  });
}

// Expose quick console helper
window.checkRecipeUnitCosts = logRecipeRowIssues;
...
function openRecipeModal(recipe = null) {
  ...
  // ensure cost preview is up-to-date when opening the modal
  refreshRecipeCostPreview();
}
...
function addRecipeRow(entry = {}) {
  ...
  row.innerHTML = `
    ...
    <div class="col-12">
      <div class="small text-muted recipe-row-costs">Unit cost: ₱0.00 · Line cost: ₱0.00</div>
    </div>
  `;
  recipeRowsContainer.appendChild(row);
  const removeBtn = row.querySelector('.remove-recipe-row-btn');
  ...
  const ingredientInput = row.querySelector('.recipe-ingredient-input');
  const qtyInput = row.querySelector('.recipe-qty-input');
  const unitInput = row.querySelector('.recipe-unit-input');
  [ingredientInput, qtyInput, unitInput].forEach(el => {
    if (!el) return;
    el.addEventListener('input', () => {
      updateRecipeRowCosts(row);
      refreshRecipeCostPreview();
    });
  });
  try { enableTouchComboboxFallback(); } catch (e) { /* ignore */ }
...
function updateRecipeRowCosts(row) {
  if (!row) return;
  const ingredientInput = row.querySelector('.recipe-ingredient-input');
  const qtyInput = row.querySelector('.recipe-qty-input');
  const costDisplay = row.querySelector('.recipe-row-costs');
  if (!ingredientInput || !qtyInput || !costDisplay) return;

  const ingredientName = String(ingredientInput.value || '').trim();
  const qty = Number(qtyInput.value || 0) || 0;
  const ingredient = ingredients.find(it => String(it.name || '').toLowerCase() === ingredientName.toLowerCase());
  const unitCost = ingredient ? Number(ingredient.unitCost || 0) : 0;
  const lineCost = unitCost * qty;
  costDisplay.textContent = `Unit cost: ${formatCurrency(unitCost)} · Line cost: ${formatCurrency(lineCost)}`;
}

function updateAllRecipeRowCosts() {
  Array.from(recipeRowsContainer.querySelectorAll('.recipe-row')).forEach(row => updateRecipeRowCosts(row));
}
...
// Provide a lightweight combobox fallback for touch devices (iPad/Safari)
function enableTouchComboboxFallback() {
  if (!isTouchDevice()) return;

  const inputs = Array.from(document.querySelectorAll('input[list]'));
  inputs.forEach(input => {
    if (input.dataset.comboboxEnhanced) return;
    const listId = input.getAttribute('list');
    const datalist = document.getElementById(listId);
    if (!datalist) return;

    const options = Array.from(datalist.options || []).map(o => o.value).filter(Boolean);
    if (!options.length) return;

    const wrapper = document.createElement('div');
    wrapper.className = 'touch-combobox-wrapper';
    ...
    dropdown.className = 'touch-combobox-dropdown d-none';
    ...
    function renderList(filter) {
      ...
      const filtered = needle ? options.filter(v => v.toLowerCase().includes(needle)) : options.slice(0, 50);
      if (!filtered.length) {
        const none = document.createElement('div');
        none.className = 'small text-muted p-2';
        none.textContent = 'No results';
        dropdown.appendChild(none);
        return;
      }

      filtered.slice(0, 200).forEach(val => {
        const item = document.createElement('div');
        item.className = 'touch-combobox-item p-2';
    ...
    input.addEventListener('input', () => {
      renderList(input.value || '');
      if (dropdown.classList.contains('d-none')) showDropdown();
    });

    input.addEventListener('focus', () => setTimeout(showDropdown, 0));
...
function isOnTheHouseOrder(order) {
  return String(order?.note || '').trim().toLowerCase() === 'on the house';
}
...
async function confirmOrder() {
  ...
  const isOnTheHouse = selectedPaymentMethod === 'On the House';
  ...
  const newOrder = {
    ...
    note: isOnTheHouse ? 'On the House' : ''
  };
...
async function loadOrders() {
  try {
    ...
    // Populate month selector for orders (months present in orders)
    if (ordersMonthSelect) {
      const months = [...new Set(orders.filter(o => o.date).map(o => o.date.slice(0, 7)))].sort().reverse();
      ordersMonthSelect.innerHTML = months.length ? months.map(m => `<option value="${m}">${m}</option>`).join('') : '<option value="">All</option>';
      if (!ordersMonthSelect.value && months.length) ordersMonthSelect.value = months[0];
    }

    // Populate week select based on selected month
    const selectedMonth = (ordersMonthSelect && ordersMonthSelect.value) || '';
    if (ordersWeekSelect) {
      const salesWeeks = selectedMonth ? getWeeklyBreakdown(selectedMonth, 'sales') : [];
      const weekLabels = salesWeeks.length ? salesWeeks.map(w => w.label) : [];
      if (weekLabels.length) {
        const prev = ordersWeekSelect.value;
        ordersWeekSelect.innerHTML = [`<option value="">All weeks</option>`].concat(weekLabels.map(l => `<option value="${l}">${l}</option>`)).join('');
        if (prev && weekLabels.includes(prev)) ordersWeekSelect.value = prev; else ordersWeekSelect.value = '';
        ordersWeekSelect.disabled = false;
      } else {
        ordersWeekSelect.innerHTML = '<option value="">All weeks</option>';
        ordersWeekSelect.value = '';
        ordersWeekSelect.disabled = true;
      }
    }

    // Filter orders by selected month and week
    let displayed = orders.slice();
    if (selectedMonth) {
      displayed = displayed.filter(o => o.date && o.date.slice(0, 7) === selectedMonth);
    }

    // If a specific week is selected, compute anchor day and filter to that week
    if (ordersWeekSelect && ordersWeekSelect.value) {
      const dated = displayed
        .map(o => ({ ...o, dayStart: toStartOfDay(o.date) }))
        .filter(e => Boolean(e.dayStart))
        .sort((a, b) => a.dayStart - b.dayStart);
      const anchorDay = dated[0]?.dayStart || null;
      if (anchorDay) {
        const targetLabel = ordersWeekSelect.value;
        displayed = displayed.filter(o => buildWeekLabelFromAnchor(o.date, anchorDay) === targetLabel);
      }
    }

    ordersTableBody.innerHTML = displayed.length
      ? displayed.map(order => `
    ...
    console.error(error);
    ordersTableBody.innerHTML = '<tr><td colspan="7" class="text-danger">Unable to load orders.</td></tr>';
  }
}
...
function renderAdminRecipes() {
  adminRecipesList.innerHTML = recipes.map(recipe => {
    const unitCost = calculateRecipeUnitCost(recipe.ingredients || []);
    return `
    <div class="col-md-6 col-xl-4">
      <div class="card admin-recipe-card h-100 shadow-sm" data-product="${recipe.productName || recipe.productId || recipe.product}" role="button" tabindex="0">
        <div class="card-body">
          ...
          <div class="recipe-ingredients">
            ${recipe.ingredients && recipe.ingredients.length ? recipe.ingredients.map(entry => `<span class="recipe-ingredient-pill">${entry.ingredient}</span>`).join('') : '<span class="text-muted small">No ingredients listed</span>'}
          </div>
          <div class="d-flex justify-content-between align-items-center mt-2">
            <div class="text-muted small">${(recipe.ingredients || []).length} ingredient${(recipe.ingredients || []).length === 1 ? '' : 's'}</div>
            <div class="text-muted small">Unit cost: ${formatCurrency(unitCost)}</div>
          </div>
        </div>
      </div>
    </div>
...
function calculateRecipeUnitCost(entries = []) {
  if (!Array.isArray(entries) || !entries.length) return 0;
  let total = 0;
  entries.forEach(entry => {
    if (!entry || !entry.ingredient) return;
    const qty = Number(entry.qty || entry.quantity || 0) || 0;
    const ingredient = ingredients.find(it => String(it.name || '').toLowerCase() === String(entry.ingredient || '').toLowerCase());
    if (!ingredient) return;
    const unitCost = Number(ingredient.unitCost || 0);
    total += qty * unitCost;
  });
...
function refreshRecipeCostPreview() {
  const display = document.getElementById('recipe-total-cost');
  if (!display) return;
  const entries = getRecipeEntries();
  const total = calculateRecipeUnitCost(entries);
...
function getWeeklyBreakdown(monthKey, type) {
  ...
  const entries = records.map(record => {
    if (type === 'ingredient') {
      const value = Number(record.costAdded || 0);
      return {
    ...
    const grossValue = Number(record.totalAmount || 0);
    const onTheHouseValue = isOnTheHouseOrder(record) ? grossValue : 0;
    return {
      dateValue: record.date,
      grossValue,
      onTheHouseValue,
      value: grossValue - onTheHouseValue
    };
  }).filter(entry => entry.grossValue > 0 || entry.value > 0 || entry.onTheHouseValue > 0);
  ...
  datedEntries.forEach(entry => {
    ...
    existing.grossValue += entry.grossValue;
    existing.onTheHouseValue += entry.onTheHouseValue;
  ...
  return [...bucketMap.values()]
    ...
    .map(entry => ({
      ...
      onTheHouseValue: entry.onTheHouseValue
    }));
}
...
function renderDashboard() {
  ...
  // Populate week select based on computed weekly buckets for ingredients
  if (dashboardWeekSelect) {
    const weekOptions = ingredientWeeks.length ? ingredientWeeks.map(w => w.label) : [];
    if (weekOptions.length) {
      // preserve previous selection when possible
      const prev = dashboardWeekSelect.value;
      dashboardWeekSelect.innerHTML = weekOptions.map(label => `<option value="${label}">${label}</option>`).join('');
      if (prev && weekOptions.includes(prev)) {
        dashboardWeekSelect.value = prev;
      } else {
        dashboardWeekSelect.value = '';
      }
      dashboardWeekSelect.disabled = false;
    } else {
      dashboardWeekSelect.innerHTML = '<option value="">All weeks</option>';
      dashboardWeekSelect.value = '';
      dashboardWeekSelect.disabled = true;
    }
  }
  ...
  const salesGrossTotal = salesWeeks.reduce((sum, entry) => sum + Number(entry.grossValue || 0), 0);
  const salesOnTheHouseTotal = salesWeeks.reduce((sum, entry) => sum + Number(entry.onTheHouseValue || 0), 0);

  ingredientDashboardSummary.textContent = formatCurrency(ingredientTotal);
  salesDashboardSummary.textContent = `${formatCurrency(salesTotal)} (${formatCurrency(salesGrossTotal)} - ${formatCurrency(salesOnTheHouseTotal)})`;
  ...
  salesDashboardChart.innerHTML = salesWeeks.length
    ? salesWeeks.map(entry => `<div class="mb-2"><div class="small text-muted">${entry.label}</div><div class="progress" style="height: 8px;"><div class="progress-bar bg-primary" style="width: ${Math.max(12, (entry.value / Math.max(...salesWeeks.map(item => item.value), 1)) * 100)}%"></div></div><div class="small mt-1">${formatCurrency(entry.value)} (${formatCurrency(Number(entry.grossValue || 0))} - ${formatCurrency(Number(entry.onTheHouseValue || 0))})</div></div>`).join('')
  ...
  if (ingredientDashboardEntriesBody) {
    if (!monthIngredientEvents.length) {
      ...
      // If a week is selected, filter events to that week
      const selectedWeekLabel = dashboardWeekSelect && dashboardWeekSelect.value ? dashboardWeekSelect.value : '';
      const filtered = selectedWeekLabel
        ? dated.filter(entry => buildWeekLabelFromAnchor(entry.timestamp, anchorDay) === selectedWeekLabel)
        : dated;

      // update the ingredient summary to reflect selected week when applicable
      if (selectedWeekLabel) {
        const weekSum = filtered.reduce((s, e) => s + Number(e.costAdded || 0), 0);
        ingredientDashboardSummary.textContent = formatCurrency(weekSum);
      }

      ingredientDashboardEntriesBody.innerHTML = filtered.length
        ? filtered
            ...
            .join('')
        : '<tr><td colspan="5" class="text-muted">No entries for selected week</td></tr>';
    }
  }

  salesDashboardTableBody.innerHTML = salesWeeks.length
    ? salesWeeks.map(entry => `<tr><td>${entry.label}</td><td>${formatCurrency(Number(entry.grossValue || 0))}</td><td>${formatCurrency(Number(entry.onTheHouseValue || 0))}</td><td>${formatCurrency(entry.value)}</td></tr>`).join('')
...
function renderReports() {
  ...
  const grossSales = filteredOrders.reduce((sum, order) => sum + Number(order.totalAmount || 0), 0);
  const onTheHouseTotal = filteredOrders.reduce((sum, order) => sum + (isOnTheHouseOrder(order) ? Number(order.totalAmount || 0) : 0), 0);
  const totalSales = grossSales - onTheHouseTotal;
  const totalExpense = getIngredientCostEvents().reduce((sum, entry) => {
    const eventDate = String(entry.timestamp || '').slice(0, 10);
    if (startDate && eventDate < startDate) return sum;
    if (endDate && eventDate > endDate) return sum;
    return sum + Number(entry.costAdded || 0);
  }, 0);
  ...
  reportSummary.innerHTML = `
    <div class="row g-3">
      ...
      <div class="col-md-4">
        <div class="border rounded p-3">
          <div class="text-muted small">On the House</div>
          <div class="fw-semibold">${formatCurrency(onTheHouseTotal)}</div>
        </div>
      </div>
      <div class="col-md-4">
        <div class="border rounded p-3">
          <div class="text-muted small">Total Expense</div>
          <div class="fw-semibold">${formatCurrency(totalExpense)}</div>
        </div>
      </div>
      <div class="col-md-4">
        <div class="border rounded p-3">
...
async function unlockApp() {
  ...
  // If Firebase isn't initialized yet, allow a local unlock fallback
  // Require Firebase Auth to be available. Do not allow local fallback.
  if (!firebaseAuth) {
    setAuthStatus('Firebase Authentication is unavailable. Provide runtime config first to sign in.', 'warning');
    // Reveal config UI so admin can paste or apply Firebase settings
    enableFirebaseConfigUi(true);
    return;
  }
...
if (dashboardWeekSelect) dashboardWeekSelect.addEventListener('change', renderDashboard);
if (ordersMonthSelect) ordersMonthSelect.addEventListener('change', () => loadOrders());
if (ordersWeekSelect) ordersWeekSelect.addEventListener('change', () => loadOrders());
...
loadRecipes();
renderCart();
// Hide Firebase config inputs until an admin unlocks the app
enableFirebaseConfigUi(false);
// Initialize Firebase Auth if a bootstrap (auth-only) config is available.
// We accept a stored bootstrap config, `window.firebaseBootstrapConfig`,
// `window.firebaseConfig` (used in `firebase-config.js`), or attempt to
// fetch a minimal bootstrap config from a public server endpoint.
async function bootstrapAuthIfNeeded() {
  let bootstrapConfig = readStoredObject('coffee-pos-firebase-bootstrap-config') || window.firebaseBootstrapConfig || window.firebaseConfig || null;
  if (!bootstrapConfig || !bootstrapConfig.apiKey || !bootstrapConfig.authDomain) {
    try {
      const fetched = await fetchBootstrapConfigFromServer();
      if (fetched) bootstrapConfig = fetched;
    } catch (e) { /* ignore */ }
  }

  if (bootstrapConfig && bootstrapConfig.apiKey && bootstrapConfig.authDomain) {
    initFirebaseAuthOnly(bootstrapConfig);
    // keep config UI hidden; full runtime config will be fetched after sign-in
    enableFirebaseConfigUi(false);
  } else {
    // No bootstrap config found; reveal config UI so admin can paste it later
    enableFirebaseConfigUi(true);
  }
}
// Attempt to bootstrap Firebase Auth (may fetch config from server).
bootstrapAuthIfNeeded().catch(() => { /* ignore */ });
isLocked = true;
lockButton.textContent = 'Sign In';
renderView('pos');
openPinScreen('pos');
