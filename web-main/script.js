...
function renderAdminIngredients() {
  adminIngredientsTableBody.innerHTML = ingredients.map(item => `
    <tr>
      ...
      <td>
        ...
        <button class="btn btn-sm btn-outline-secondary me-2 edit-ingredient-btn" data-id="${item.ingredientId}" type="button">Edit</button>
        <button class="btn btn-sm btn-outline-warning toggle-ingredient-status-btn" data-id="${item.ingredientId}" type="button">${item.isActive === false ? 'Activate' : 'Deactivate'}</button>
      </td>
    </tr>
  `).join('');
  ...
  adminIngredientsTableBody.querySelectorAll('.edit-ingredient-btn').forEach(button => {
    button.addEventListener('click', () => {
      ...
      ingredientIdInput.value = ingredient.ingredientId;
      ingredientNameInput.value = ingredient.name;
      ingredientCategoryInput.value = ingredient.category || '';
      ingredientStockInput.value = ingredient.currentStock || 0;
      ingredientUnitInput.value = ingredient.unit || '';
      ingredientBaseQtyInput.value = ingredient.baseQuantity || 0;
      ingredientBaseCostInput.value = ingredient.baseCost || 0;
    });
  });

  adminIngredientsTableBody.querySelectorAll('.toggle-ingredient-status-btn').forEach(button => {
    button.addEventListener('click', () => {
      const ingredient = ingredients.find(item => item.ingredientId === button.dataset.id);
      if (!ingredient) return;
      ingredient.isActive = ingredient.isActive === false;
      saveJsonData(ingredientsStorageKey, ingredients);
      renderAdminIngredients();
      renderIngredientFilters();
      renderIngredients();
